...
def saitekika(file):
  ...
  #目的関数の設定
  model.objective = minimize(100*xsum(y[d,t] for d in D for t in T)+xsum(h[i] for i in I if i not in syain)-0.1*xsum(syuhuten[i,d,t]*x[i,d,t] for i in I for d in D for t in T if i not in syain))
