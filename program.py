...
def saitekika(file):
  ...
  for i in range(len(I)):
    ...
    for d in D:
      ...
      if sihutokibou == "ABCDEF":
        ...
        k[n,d,4] = 0
        k[n,d,5] = 0
      #print(k[n,d,1],k[n,d,2],k[n,d,3],k[n,d,4],k[n,d,5])
  ...
  #目的関数の設定
  model.objective = minimize(xsum(y[d,t] for d in D for t in T))
  ...
  #1回目の最適化の結果を用いて，2回目の最適化を実行する
  if status == OptimizationStatus.OPTIMAL:
      print('1回目の最適値 =', model.objective_value)
      ...
      model += xsum(y[d,t] for d in D for t in T) == V

      model.objective = minimize(xsum(h[i] for i in I if i not in syain)-0.1*xsum(syuhuten[i,d,t]*x[i,d,t] for i in I for d in D for t in T if i not in syain))
