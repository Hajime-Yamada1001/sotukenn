...
def saitekika(file):
  ...
  for i,row in enumerate(sheethope.iter_rows()):
      for j,cell in enumerate(row):
        value = ""
        if not (i+1 >= 4 and i+1<=36 and j+1 >= 2 and j+1 <= kongetunissu+1):
          value = cell.value
  ...
  #その時間帯に必要な人数を割り当てる
  for d in D:
      model += xsum(x[i,d,1] + x[i,d,4] for i in I) == p[d,1] - y[d,1]
      model += xsum(x[i,d,2] + x[i,d,4] + x[i,d,5] for i in I) == p[d,2] - y[d,2]
      model += xsum(x[i,d,3] + x[i,d,5] for i in I) == p[d,3] - y[d,3]
  ...
  #目的関数の設定
  model.objective = minimize(xsum(y[d,t] + e[d,t] for d in D for t in T)+xsum(h[i] for i in I)-xsum(syuhuten[i,d,t]*x[i,d,t] for i in I for d in D for t in T))
